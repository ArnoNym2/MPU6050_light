...
MPU6050::MPU6050(TwoWire &w, boolean _filterOn, SimpleKalmanFilter  _filter): filter(_filter)
{
  wire = &w;
...
void MPU6050::update()
{
  ...
  angleAccY = -atan2(accX, sgZ * sqrt(accZ * accZ + accY * accY)) * RAD_2_DEG; // [- 90,+ 90] deg // not any more, see below

  angleAccYFiltered = filter.updateEstimate(angleAccY);
  unsigned long Tnew = millis();
  float dt = (Tnew - preInterval) * 1e-3;
  preInterval = Tnew;

  angleAccY = (filterOn == true ? angleAccYFiltered : angleAccY);
  ...
  angleY = wrap(filterGyroCoef * (angleAccY + wrap(angleY + sgZ * gyroY * dt - angleAccY, 180)) + (1.0 - filterGyroCoef) * angleAccY, 180); // Also wrapping to +/- 180 deg
